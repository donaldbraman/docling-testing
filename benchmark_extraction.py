...
from extract_body_only import is_likely_citation, calculate_citation_density
...
def extract_and_measure(pdf_path: Path, config: BenchmarkConfig) -> BenchmarkMetrics:
    ...
    high_density = sum(1 for p in body_paragraphs
                      if len(p) > 200 and calculate_citation_density(p) > 0.15)
    ...
    metrics = BenchmarkMetrics(
        ...
        removal_percentage=round(100 * (1 - len(body_only.split()) / len(all_text.split())), 2) if len(all_text.split()) > 0 else 0,
...
"""
    ...
    for m in sorted(all_metrics, key=lambda x: (x.document_name, x.config_name)):
        pages_per_min = m.page_count / m.processing_time_minutes if m.processing_time_minutes > 0 else 0
    ...
    for config_name, metrics_list in sorted(by_config.items()):
        ...
        avg_citations = sum(m.citations_caught_by_heuristic for m in metrics_list) / len(metrics_list)
