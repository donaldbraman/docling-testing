...
def diagnose_labels(pdf_path: Path):
    ...
    print(f"\n💡 Key insight:")
    print(f"   Docling labeled these citations as '{list(label_summary.keys())[0] if label_summary else 'unknown'}'")
    print(f"   instead of 'footnote', so they passed through our initial filter.")
    print(f"   Our heuristic should catch them based on citation patterns.")
