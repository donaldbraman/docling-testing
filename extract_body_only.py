...
def is_likely_citation(text: str) -> bool:
    ...
    if len(text) < 100 and re.search(r'\d+.*?(U\.S\.|S\. ?Ct\.|F\.\d?d|P\.\d?d).*?\d+.*?\(\d{4}\)', text[:80]):
        return True

    # Items that start with common citation signals (short items only)
    if len(text) < 200 and re.match(r'^(See|Id\.|Ibid\.|Compare|But see|Cf\.)', text, re.IGNORECASE):
        return True
...
def main():
    ...
    print(f"Hardware: M1 Pro with 32GB RAM")
    print()
    ...
    if default_metrics and optimized_metrics:
        ...
        print("-" * 80)
        print(f"{'Processing time (s)':<30} {default_metrics['elapsed_seconds']:<20} {optimized_metrics['elapsed_seconds']:<20}")
        print(f"{'Footnotes detected':<30} {default_metrics['label_counts'].get('footnote', 0):<20} {optimized_metrics['label_counts'].get('footnote', 0):<20}")
        print(f"{'Body text words':<30} {default_metrics['body_text_words']:<20,} {optimized_metrics['body_text_words']:<20,}")
        print(f"{'Footnote words removed':<30} {default_metrics['footnote_text_words']:<20,} {optimized_metrics['footnote_text_words']:<20,}")
        print(f"{'Hyphenation (body)':<30} {default_metrics['hyphen_body']:<20} {optimized_metrics['hyphen_body']:<20}")
