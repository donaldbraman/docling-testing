        ...
        if result.pages:
            ...
            for attr in ['predictions', 'layout', 'elements', 'boxes', 'annotations']:
                if hasattr(first_page, attr):
                    ...
                    if val:
                        print(f"      Object attributes: {[a for a in dir(val) if not a.startswith('_')][:15]}")

                        # Try to iterate or access common properties
                        try:
                            # Check if it's iterable
                            items = list(val) if hasattr(val, '__iter__') and not isinstance(val, str) else None
                            if items:
                                print(f"      ✅ Iterable! Found {len(items)} items")
                                if items:
                                    first_elem = items[0]
                                    print(f"        First element type: {type(first_elem)}")
                                    print(f"        First element attributes: {[a for a in dir(first_elem) if not a.startswith('_')][:15]}")

                                    # Look for label
                                    for label_attr in ['label', 'class_name', 'type', 'category', 'class_id']:
                                        if hasattr(first_elem, label_attr):
                                            print(f"          ✅ Label: {label_attr} = {getattr(first_elem, label_attr)}")

                                    # Look for bbox
                                    for bbox_attr in ['bbox', 'box', 'coordinates', 'geometry', 'bounding_box']:
                                        if hasattr(first_elem, bbox_attr):
                                            print(f"          ✅ Bbox: {bbox_attr} = {getattr(first_elem, bbox_attr)}")
