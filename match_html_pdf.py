...
def extract_from_html(html_file: Path) -> List[Paragraph]:
    ...
    if total_p_tags < 20:
        print(f"  ⚠️  Skipping {html_file.name}: JavaScript-rendered content ({total_p_tags} <p> tags)")
        return []
    ...
    footnote_containers = ['.footnote', 'aside', '[role="note"]', '.note', '#footnotes', '#footnote_wrapper']
    ...
    for i, p in enumerate(soup.find_all('p')):
        ...
        for selector in footnote_containers:
            if p.find_parent(class_=selector.strip('.#[]')) or \
               p.find_parent(id=selector.strip('#')) or \
