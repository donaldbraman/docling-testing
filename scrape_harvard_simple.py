...
def scrape_article(url: str, page, html_dir: Path, pdf_dir: Path) -> bool:
    ...
    try:
        ...
        try:
            download_button = page.query_selector('.single-article__header-download-button a[download]')
            if download_button:
                href = download_button.get_attribute('href')
                if href and 'Factsheet' not in href:  # Skip factsheet
                    pdf_url = href if href.startswith('http') else f"https://harvardlawreview.org{href}"
        except:
            pass
        ...
        print(f"    ✓ Complete pair downloaded!")
        return True
