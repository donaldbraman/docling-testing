...
class LawReviewScraper:
    ...
    def discover_articles_via_search(self, journal_key: str, config: Dict) -> List[Dict]:
        ...
        for query in search_queries[:1]:  # Just try first query to avoid overload
            ...
            for container in soup.find_all(['article', 'div'], class_=re.compile(r'article|post|entry')):
    ...
    def find_pdf_url(self, html_url: str, soup: BeautifulSoup, config: Dict) -> Optional[str]:
    ...
    def scrape_journal(self, journal_key: str, target_articles: int = 5) -> int:
        ...
        if not articles:
            print(f"  ✗ No articles discovered")
            return 0
    ...
    def scrape_multiple_journals(
        ...
        print(f"{'='*80}")
        print(f"LAW REVIEW ARTICLE SCRAPER")
        print(f"{'='*80}")
        ...
        if self.stats['pairs_complete'] >= target_total * 0.7:
            ...
            print(f"  Ready for label transfer: python match_html_pdf.py")
        else:
            print(f"\n⚠️  Only {self.stats['pairs_complete']} pairs collected")
            print(f"  May need to adjust scraping strategy or add more journals")
