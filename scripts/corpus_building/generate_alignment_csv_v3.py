#!/usr/bin/env python3
"""
Generate CSV files showing PDF-to-HTML alignment for visual inspection (V3 - MATCH AMBIGUITY).

This version tests a SINGLE hypothesis: match ambiguity filtering.

Instead of multiple overlapping rules (length filters, section headers, dynamic thresholds),
this version uses ONE simple concept:

    If multiple HTML paragraphs score similarly high for the same PDF text,
    it's AMBIGUOUS → filter it as a false positive.

This is conceptually simpler and computationally free (we already compute all scores).
...
- ambiguity_score: Number of competing matches above threshold
...
Usage:
    python generate_alignment_csv_v3.py [--margin MARGIN]
...
Arguments:
    --margin: Minimum score margin between best and 2nd best match to accept (default: 5)
              ...
              Experiment with values: 3, 5, 10, 15, etc.

Author: Claude Code
Date: 2025-01-19 (v3 with match ambiguity filtering)
"""
...
def find_best_match_with_uniqueness(
    pdf_text: str,
    ...
    """
    Find best matching HTML paragraph for PDF text with ambiguity filtering (V3).

    The ONLY filtering rule: If multiple HTML paragraphs score similarly high,
    it's ambiguous → reject the match.
...
def generate_alignment_csv(basename: str, output_dir: Path, ambiguity_margin: int = 5):
    ...
    for pdf_item in pdf_texts:
        ...
        body_idx, body_score, body_matched, body_competitors = find_best_match_with_uniqueness(
            pdf_text, body_paras, ambiguity_margin=ambiguity_margin
        )

        # Try matching to footnotes
        fn_idx, fn_score, fn_matched, fn_competitors = find_best_match_with_uniqueness(
            pdf_text, footnote_paras, ambiguity_margin=ambiguity_margin
        )

        # Track if filtered due to ambiguity
        if (body_idx == -1 and body_competitors > 1) or (fn_idx == -1 and fn_competitors > 1):
        ...
        if body_idx >= 0 and fn_idx >= 0:
            ...
            if body_score > fn_score:
                ...
                ambiguity_score = body_competitors
                ...
                ambiguity_score = fn_competitors
            ...
            ambiguity_score = max(body_competitors, fn_competitors)
            csv_rows.append(
                {
                    "page_no": page_no,
                    "pdf_text": pdf_text,
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate alignment CSVs with match ambiguity filtering (V3)"
    )
    parser.add_argument(
        "--margin",
        type=int,
    ...
    output_dir = Path("data/v3_data/v3_csv_v3")
    processed_html_dir = Path("data/v3_data/processed_html")

    html_files = sorted(processed_html_dir.glob("*.json"))

    print(f"{'=' * 80}")
    print(f"Generating alignment CSVs (V3 - MATCH AMBIGUITY) for {len(html_files)} articles")
    print(f"{'=' * 80}")
    print()
    print("Testing ONE hypothesis:")
    print("  - Match ambiguity filtering: If multiple HTML paragraphs score")
    print("    similarly high, it's ambiguous → filter as false positive")
    print()
    print("NO other filters:")
    print("  - No minimum text length filter")
    print("  - No section header detection")
    print("  - No dynamic thresholds")
