#!/usr/bin/env python3
"""
Generate CSV files showing PDF-to-HTML alignment for visual inspection (V5 - SEQUENTIAL LOCALITY).

This version tests the SEQUENTIAL LOCALITY hypothesis:

Documents have inherent sequential structure. The next PDF line should match HTML
paragraphs that appear AFTER the last match, not before it.

Key insight: Prevents false positives from similar text appearing earlier in the document
(e.g., similar footnote structures like "see, e.g., Thomas supra note __").

Approach:
- Track last_body_match_index and last_footnote_match_index
- Only search FORWARD from the last match
- Naturally disambiguates similar patterns by document position
...
Author: Claude Code
Date: 2025-01-19 (v5 with sequential locality bias)
"""
...
def find_best_match_sequential(
    ...
    """
    Find best matching HTML paragraph with SOFT locality bias + windowed search (V5).

    Locality assumption: Documents roughly follow sequential order.
    - First match: Search first N characters (assumes doc starts in order)
    - Subsequent matches: Search within window around last match
    - When multiple matches have similar scores, prefer the one closer to the last match
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate alignment CSVs with sequential locality bias (V5)"
    )
    parser.add_argument(
        "--threshold",
        type=int,
    ...
    output_dir = Path(f"data/v3_data/v3_csv_v5{output_suffix}")
    ...
    print(f"{'=' * 80}")
    print(f"Generating alignment CSVs (V5 - SEQUENTIAL LOCALITY) for {len(test_articles)} articles")
    print(f"{'=' * 80}")
    print()
    print("Testing SOFT LOCALITY BIAS + WINDOWED SEARCH hypothesis:")
        ...
        try:
            ...
            if stats:
                print(
                    ...
                    f"{stats['filtered_sequential']} no forward match)"
                )
    ...
    print(f"  No forward match found: {total_stats['filtered_sequential']:,}")
