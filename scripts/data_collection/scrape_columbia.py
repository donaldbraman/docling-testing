...
class ColumbiaLawReviewScraper:
                ...
                for path in ["/issues", "/archive", "/articles"]:
                    try:
                        await self.page.goto(
                            f"{self.BASE_URL}{path}", wait_until="networkidle", timeout=20000
                        )
    ...
    async def download_article_pair(self, article: dict[str, str]) -> bool:
        ...
        if html_path.exists() and pdf_path.exists():
            if (
                html_path.stat().st_size >= self.MIN_HTML_SIZE
                and pdf_path.stat().st_size >= self.MIN_PDF_SIZE
            ):
        ...
        try:
            ...
            if len(html_content) < self.MIN_HTML_SIZE:
                print(
                    f"    ✗ HTML too small: {len(html_content)} bytes (need {self.MIN_HTML_SIZE})"
                )
                return False
    ...
    async def find_pdf_link(self) -> str | None:
