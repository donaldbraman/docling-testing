...
def fetch_article(article_data):
    ...
    try:
        ...
        # Count footnotes
        footnote_count = len(
            re.findall(r"modern-footnotes-footnote|<sup[^>]*>\s*\d+\s*</sup>", html_content, re.I)
        )
        ...
        for pattern in pdf_patterns:
            ...
            if matches:
                ...
                for match in matches:
                    ...
                    if any(
                        domain in match for domain in ["worldbank.org", "ojp.gov", "justice.gov"]
                    ):
                        continue
                    pdf_url = match
                    break
        ...
        if not pdf_url:
            ...
            download_match = re.search(
                r'<a[^>]*href="([^"]*)"[^>]*>.*?(?:download|view\s*pdf).*?</a>',
                html_content,
                re.I | re.S,
            )
            if download_match:
        ...
        if pdf_url:
            ...
            print(
                f"SUCCESS: Saved HTML ({html_size / 1024:.1f} KB) and PDF ({pdf_size / 1024:.1f} KB)"
            )
            return True
        else:
