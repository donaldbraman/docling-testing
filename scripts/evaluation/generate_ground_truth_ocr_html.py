#!/usr/bin/env python3
"""
Generate side-by-side HTML comparison of ground truth vs Docling OCR.

Usage:
  python3 scripts/evaluation/generate_ground_truth_ocr_html.py
"""
...
def align_paragraphs(paras1: list[str], paras2: list[str]) -> list[tuple[str, str, float]]:
    ...
    while i < len(paras1):
        ...
        # Look ahead up to 50 paragraphs
...
<head>
    ...
    <title>Ground Truth vs Docling OCR</title>
    <style>
        body {{
...
<body>
    <div class="header">
        <h1>Ground Truth vs Docling OCR Comparison</h1>
        <div class="metadata">
            <div><strong>Document:</strong> {metadata["document"]}</div>
            <div><strong>Ground Truth:</strong> All HTML paragraphs from Westlaw</div>
            <div><strong>OCR:</strong> Docling (ocrmac engine) on image-only PDF</div>
        </div>
        <div class="stats">
            <div class="stat-box">
                <div class="stat-label">Ground Truth</div>
                <div class="stat-value">{metadata["gt_words"]:,} words</div>
            </div>
            <div class="stat-box">
                <div class="stat-label">Docling OCR</div>
                <div class="stat-value">{metadata["ocr_words"]:,} words</div>
            </div>
            <div class="stat-box">
            ...
            <div class="stat-box">
                <div class="stat-label">Aligned Pairs</div>
    ...
    <div class="legend">
        ...
        <div class="legend-items">
            ...
            <div class="legend-item">
                ...
                <span>Only in Ground Truth</span>
            </div>
            <div class="legend-item">
                <mark class="diff-insert">Green</mark>
                <span>Only in Docling OCR</span>
            </div>
            <div class="legend-item">
...
"""
    ...
    for idx, (para1, para2, similarity) in enumerate(aligned_paras, 1):
        ...
        if para1 and para2:
            ...
            para1_html = para1 if para1 else "<em>(Missing in Ground Truth)</em>"
            para2_html = para2 if para2 else "<em>(Missing in Docling OCR)</em>"
...
def main():
    ...
    output_path = Path("results/ground_truth_vs_ocr.html")

    print("Loading ground truth...")
    gt_data = load_json(gt_path)

    print("Loading Docling OCR...")
    ocr_data = load_json(ocr_path)

    # Extract ALL paragraphs from ground truth (not just body-text)
    gt_paras = [normalize_text(p["text"]) for p in gt_data["paragraphs"]]

    # Get Docling text blocks and normalize
    ocr_paras = [normalize_text(t) for t in ocr_data.get("texts", []) if t.strip()]

    # Filter out very short blocks
    ocr_paras = [p for p in ocr_paras if len(p) > 10]

    print(f"  Ground truth: {len(gt_paras)} paragraphs (all classes)")
    print(f"  Docling OCR: {len(ocr_paras)} text blocks")

    print("Aligning paragraphs...")
    aligned = align_paragraphs(gt_paras, ocr_paras)

    # Calculate metadata
    gt_words = sum(len(p.split()) for p in gt_paras)
    ocr_words = sum(len(p.split()) for p in ocr_paras)

    metadata = {
        "document": "usc_law_review_in_the_name_of_accountability.pdf",
        "gt_words": gt_words,
        "ocr_words": ocr_words,
        "diff_words": abs(ocr_words - gt_words),
    }
    ...
    print(f"  Docling OCR: {ocr_words:,} words")
    print(
        f"  Difference: {metadata['diff_words']:,} words ({abs(ocr_words - gt_words) / gt_words * 100:.1f}%)"
    )
