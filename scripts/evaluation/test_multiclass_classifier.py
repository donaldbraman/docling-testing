...
def classify_text(text: str, tokenizer, model, id_to_label) -> tuple[str, float]:
    ...
    # Tokenize
    inputs = tokenizer(
        text, return_tensors="pt", padding="max_length", truncation=True, max_length=1024
    )
...
def test_on_samples(tokenizer, model, id_to_label):
    ...
    samples = [
        (
            "body_text",
            "The Supreme Court held that the government must provide compensation when taking private property for public use. This principle, established in the Fifth Amendment, forms the foundation of takings jurisprudence.",
        ),
        (
            "body_text",
            "In recent decades, criminal justice reform has emerged as a critical policy priority. Scholars have identified prosecutorial discretion as a key factor in mass incarceration.",
        ),
        ...
        (
            "footnote",
            "15 For a comprehensive discussion of background principles, see Rose, Property as Storytelling, 2 YALE J.L. & HUMAN. 37, 52-54 (1990).",
        ),
        (
            "caption",
            "Figure 1. Distribution of prosecutorial decisions by jurisdiction, showing significant variation across counties (N=3,247 cases).",
        ),
        (
            "caption",
            "Table 2. Summary statistics for conviction rates by offense type, controlling for demographic factors.",
        ),
        (
            "page_header",
            "HARVARD LAW REVIEW                                                  [Vol. 135:123",
        ),
        (
            "page_footer",
            "2024]           PROSECUTORIAL DISCRETION AND REFORM                         147",
        ),
        (
            "cover",
            "Downloaded from HeinOnline at 10.123.45.67 on 2024-01-15. Citation: 135 Harv. L. Rev. 123 (2021). Bluebook 21st ed.: Author, Title, 135 HARV. L. REV. 123 (2021).",
        ),
        (
            "cover",
            "JSTOR is a not-for-profit service. Your use of the JSTOR archive indicates acceptance of Terms and Conditions of Use. Accessed: 2024-01-15 from www.jstor.org",
        ),
    ]
    ...
    for expected_label, text in samples:
        ...
        print(
            f"{match} Expected: {expected_label:12s} | Predicted: {pred_label:12s} (conf: {confidence:.3f})"
        )
...
def test_on_pdf(pdf_path: Path, tokenizer, model, id_to_label):
    ...
    print("Classifying first 20 paragraphs:\n")
...
def main():
    ...
    print("=" * 80)
    print("\nNext steps:")
    print("  1. Review classification results above")
    print("  2. Test on more PDFs if needed")
