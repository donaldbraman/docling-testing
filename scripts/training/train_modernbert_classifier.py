...
def load_and_prepare_data(corpus_path: Path):
    ...
    print("\nDataset splits:")
    print(
        f"  Train: {len(X_train):,} ({sum(y_train):,} footnotes, {len(y_train) - sum(y_train):,} body)"
    )
    print(f"  Val:   {len(X_val):,} ({sum(y_val):,} footnotes, {len(y_val) - sum(y_val):,} body)")
    print(
        f"  Test:  {len(X_test):,} ({sum(y_test):,} footnotes, {len(y_test) - sum(y_test):,} body)"
    )
...
def evaluate_model(trainer, test_dataset, output_dir: Path):
    ...
    print(classification_report(y_true, y_pred, target_names=["body_text", "footnote"], digits=3))
    ...
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=["Body Text", "Footnote"])
...
def main():
    ...
    try:
        ...
        print("\nNext steps:")
        print("  1. Test model on PDFs: python test_modernbert_classifier.py")
        print("  2. Integrate into pipeline: Update extract_body_only.py")
        print("  3. Compare with heuristic baseline")
