...
def evaluate_model(trainer, test_dataset, output_dir: Path):
    ...
    plt.title(
        f"DoclingBert v2: 7-Class Document Structure Classifier\nF1 Macro={f1_macro:.3f}, Accuracy={accuracy:.3f}"
    )
...
def main():
    ...
    try:
        ...
        trainer, tokenizer, model = train_model(
            train_dataset, val_dataset, class_weights, output_dir
        )
        ...
        print("\nNext steps:")
        print("  1. Test model: python test_multiclass_classifier.py")
        print("  2. Create PR for Issue #7")
