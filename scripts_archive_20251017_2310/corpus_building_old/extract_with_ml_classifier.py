...
def extract_body_text_ml(
    pdf_path: Path, model_path: Path | None = None, confidence_threshold: float = 0.5
) -> dict:
    ...
    print(
        f"  Body text:  {len(body_paragraphs)} paragraphs ({100 - stats['footnote_percentage']:.1f}%)"
    )
...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python extract_with_ml_classifier.py <pdf_path> [model_path] [confidence_threshold]"
        )
        print("\nExample:")
