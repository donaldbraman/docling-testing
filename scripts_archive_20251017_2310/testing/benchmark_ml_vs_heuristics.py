...
def calculate_accuracy(predictions: list[bool], ground_truth: list[str]) -> dict:
    ...
    # Calculate metrics
    correct = sum(p == gt for p, gt in zip(predictions, gt_bool, strict=False))
    accuracy = correct / len(predictions)

    # Confusion matrix
    tp = sum(p and gt for p, gt in zip(predictions, gt_bool, strict=False))
    fp = sum(p and not gt for p, gt in zip(predictions, gt_bool, strict=False))
    tn = sum(not p and not gt for p, gt in zip(predictions, gt_bool, strict=False))
    fn = sum(not p and gt for p, gt in zip(predictions, gt_bool, strict=False))
...
def main():
    ...
    print("\nConfusion Matrix:")
    print(
        f"  TP: {heuristic_accuracy['true_positives']:3d}  FP: {heuristic_accuracy['false_positives']:3d}"
    )
    print(
        f"  FN: {heuristic_accuracy['false_negatives']:3d}  TN: {heuristic_accuracy['true_negatives']:3d}"
    )
    ...
    if accuracy_improvement > 0.05 and ml_results["avg_time_ms"] < 50:
        ...
        print(
            f"  ({ml_results['avg_time_ms']:.1f}ms vs {heuristic_results['avg_time_ms']:.3f}ms per paragraph)"
        )
